...
class HJPeripheralAdapter: NSObject {
    ...
    /// 当前蓝牙状态
    var state: Variable<CBState> = Variable<CBState>(.unknown)
    
    /// 交互消息数组
    var message:Variable<MessageModel> = Variable<MessageModel>(MessageModel())
    
    /// 外围设备订阅状态
    var peripheralSubscribeType: Variable<SubscribeWriteType> = Variable<SubscribeWriteType>(.Type_defaule)
    ...
    func creatPeriphera(){
        ...
        self.state.asObservable().asObservable().subscribe(onNext: { model in
